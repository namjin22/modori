...
테스트를 임의로 고치지 않았다. 사용자 승인을 받으면 다음과 같이 각 할 일 줄 안에서
체크박스를 찾도록 바꾸면 된다. 동시에 누르는 의도는 그대로 유지된다.

```ts
const rows = ["첫 번째 일", "두 번째 일"].map((content) =>
  page.getByRole("listitem").filter({ hasText: content }),
);
await Promise.all(
  rows.map((row) => row.getByRole("button", { name: "완료", exact: true }).click()),
);
```
